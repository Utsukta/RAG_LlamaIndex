...
class App:
   ...
   def upload_file(self):
        ...
        if file is not None:
           ...
           if file_path:
              files=SimpleDirectoryReader(input_files=[file_path])
              loaded_file=files.load_data()
              return loaded_file
        return None
   ...
   def node_parser(self):
      Settings.text_splitter=SentenceSplitter(chunk_size=1500,chunk_overlap=20)
   ...
   def llm_model(self):
      ...
      #For Ollama
      Settings.llm=Ollama(model="llama3",request_timeout=400,temperature=0.2,top_p=0.5,top_k=40)
      return Settings.llm
...
# )
      ...
      query_engine=vector_index.as_query_engine(response_mode=response_mode,similarity_top_k=10,verbose=True,node_postprocessors=[
