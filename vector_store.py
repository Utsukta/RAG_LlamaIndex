...
class ChatwithDocument:
   ...
   def generate_rag_pipeline(self,file,llm,embed_model,node_parser,response_mode,vector_store):
      ...
      query_engine=vector_index.as_query_engine(response_mode=response_mode,verbose=True,similarity_top_k=10)
      ...
      if st.session_state.get('pipeline_generated',False):
        ...
        if prompt := st.chat_input("Enter your query", key='query'):
            ...
            if 'hyde_query_engine' in st.session_state:
               system_prompt = ("You are an AI assistant specialized in providing information from the uploaded document. "
               ...
               message = st.session_state['hyde_query_engine'].query(query_with_prompt)
